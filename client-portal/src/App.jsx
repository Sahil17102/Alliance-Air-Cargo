...
function Dashboard({ user, shipments }) {
  const active=shipments.filter(x=>x.status!=='Delivered').length
  return <div className="fade"><PageTitle eyebrow="Overview" title={`Good morning, ${user.name.split(' ')[0]}.`} text="Here’s what is happening across your cargo account today." action={<button onClick={()=>go('booking')} className="btn-primary"><Plus size={16}/> New booking</button>}/>
    {user.status==='Pending approval'&&<div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800"><Clock3 className="shrink-0" size={19}/><div><strong>Business verification pending</strong><p className="mt-1 text-xs leading-5">Your static registration is saved. Super Admin approval will activate when the third application is built.</p></div></div>}
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[[Box,'Active shipments',active,'+2 this week','bg-blue-50 text-brand'],[PackageCheck,'Delivered',shipments.filter(x=>x.status==='Delivered').length,'This month','bg-green-50 text-green-600'],[CircleDollarSign,'Quotes saved',6,'Last 30 days','bg-amber-50 text-amber-600'],[Gauge,'On-time rate','98.4%','Across all routes','bg-purple-50 text-purple-600']].map(([Icon,l,n,s,c])=><div key={l} className="card p-5"><div className={`grid h-10 w-10 place-items-center rounded-xl ${c}`}><Icon size={20}/></div><strong className="font-display mt-5 block text-2xl text-navy">{n}</strong><span className="mt-1 block text-xs font-semibold text-slate-600">{l}</span><small className="mt-2 block text-[10px] text-slate-400">{s}</small></div>)}</div>
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.35fr_.65fr]"><div className="card overflow-hidden"><div className="flex items-center justify-between border-b border-slate-100 p-5"><div><h2 className="font-display text-base font-semibold text-navy">Recent shipments</h2><p className="mt-1 text-xs text-slate-400">Latest booking activity</p></div><button onClick={()=>go('shipments')} className="text-xs font-bold text-brand">View all</button></div><ShipmentTable shipments={shipments.slice(0,4)} compact/></div><div className="card p-5"><h2 className="font-display text-base font-semibold text-navy">Quick actions</h2><div className="mt-4 space-y-2">{[[Calculator,'Compare cargo rates','rates'],[Search,'Track an AWB','track'],[Upload,'Upload documents','documents'],[Headphones,'Contact cargo desk','profile']].map(([Icon,l,r])=><button key={l} onClick={()=>go(r)} className="flex w-full items-center justify-between rounded-xl border border-slate-100 p-3 text-left text-xs font-semibold text-slate-600 transition hover:border-blue-200 hover:bg-sky hover:text-brand"><span className="flex items-center gap-3"><Icon size={17}/>{l}</span><ChevronRight size={15}/></button>)}</div><div className="mt-5 rounded-xl bg-navy p-4 text-white"><p className="text-[10px] uppercase tracking-wider text-blue-300">Cargo desk</p><strong className="mt-2 block text-sm">Need operational help?</strong><a href="tel:+919810080808" className="mt-3 flex items-center gap-2 text-xs text-blue-200"><Phone size={14}/> +91 98100 80808</a></div></div></div>
  </div>
}
