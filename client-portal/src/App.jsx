...
import {
  ...
  Plus, RefreshCcw, Search, Send, Settings, ShieldCheck, Trash2, Truck, Upload, User, UserPlus,
  Users, WalletCards, Warehouse, X, Zap,
} from 'lucide-react'
...
const canAccessRoute = (user, route) => {
  ...
  if (['dashboard','rates','profile'].includes(route)) return true
  if (route === 'booking') return permissions.includes('createBooking')
  if (route === 'shipments' || route === 'documents') return permissions.includes('viewShipments')
  if (route === 'track') return permissions.includes('trackShipments')
  return false
}
...
function Register({ onComplete, onBack, toast }) {
  ...
  const submit=async e=>{e.preventDefault();setSubmitting(true);setError('');try{const result=await api.post('/api/auth/register',{...data,role:'agent',panIndia});const account=result.account;localStorage.setItem('aac_registered',JSON.stringify(account));toast('Registration submitted. It is now visible in Super Admin approvals.');onComplete(account)}catch(requestError){setError(requestError.message||'Registration could not be submitted. Please try again.')}finally{setSubmitting(false)}}
...
function Sidebar({ route, mobile, setMobile, onLogout, user }) {
  return <><div onClick={()=>setMobile(false)} className={`fixed inset-0 z-40 bg-navy/45 backdrop-blur-sm lg:hidden ${mobile?'block':'hidden'}`}/><aside className={`fixed inset-y-0 left-0 z-50 flex w-[270px] flex-col bg-navy text-white transition-transform lg:translate-x-0 ${mobile?'translate-x-0':'-translate-x-full'}`}>
    ...
    <div className="px-4 py-5"><p className="px-3 text-[9px] font-bold uppercase tracking-[.18em] text-blue-300/60">Workspace</p><nav className="mt-3 space-y-1">{Object.entries(routeMap).filter(([key])=>canAccessRoute(user,key)).map(([key,{title,icon:Icon}])=><a key={key} href={`#/${key}`} onClick={()=>setMobile(false)} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition ${route===key?'bg-brand text-white shadow-lg shadow-blue-950/20':'text-blue-100/70 hover:bg-white/10 hover:text-white'}`}><Icon size={18}/>{key==='profile'&&user.role==='employee'?'My profile':title}</a>)}</nav></div>
...
function Dashboard({ user, shipments, wallet }) {
  ...
  const canBook=user.role==='agent'||(user.permissions||[]).includes('createBooking')
  const submitTrack=e=>{e.preventDefault();if(trackAwb.trim())go(`track?awb=${encodeURIComponent(trackAwb.trim())}`)}

  return <div className="fade">
    <PageTitle eyebrow={user.role==='employee'?'Employee operations':'Customer control centre'} title={`Good morning, ${user.name.split(' ')[0]}.`} text={user.role==='employee'?'Your workspace only shows the order actions assigned by the client account owner.':'Track operations, manage bookings and stay ahead of every cargo movement from one workspace.'} action={<div className="flex flex-wrap gap-2"><button onClick={()=>go('rates')} className="btn-secondary"><Calculator size={16}/> Check rates</button>{canBook&&<button onClick={()=>go('booking')} className="btn-primary"><Plus size={16}/> New booking</button>}</div>}/>

    {/pending/i.test(String(user.status||''))&&<div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800"><Clock3 className="shrink-0" size={19}/><div><strong>Business verification pending</strong><p className="mt-1 text-xs leading-5">Your registration is saved in Super Admin approvals. Booking access activates immediately after approval.</p></div></div>}
...
function ShipmentTable({ shipments, onDelete, canDelete=false }) {
  return <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left"><thead><tr className="bg-slate-50 text-[9px] uppercase tracking-wider text-slate-400">{['AWB number','Route','Commodity','Weight','Status'].map(x=><th key={x} className="px-5 py-3 font-bold">{x}</th>)}{canDelete&&<th className="px-5 py-3 font-bold">Action</th>}</tr></thead><tbody className="divide-y divide-slate-100">{shipments.map(s=><tr key={s.awb} className="text-xs hover:bg-slate-50"><td className="px-5 py-4 font-bold text-brand">{s.awb}</td><td className="px-5 py-4 font-semibold text-navy">{s.origin} <span className="mx-1 text-slate-300">→</span> {s.destination}</td><td className="px-5 py-4 text-slate-500">{s.commodity}</td><td className="px-5 py-4 text-slate-500">{s.weight} kg</td><td className="px-5 py-4"><StatusBadge status={s.status}/></td>{canDelete&&<td className="px-5 py-4"><button disabled={!['Booked','Cancelled'].includes(s.status)} onClick={()=>onDelete(s)} title={['Booked','Cancelled'].includes(s.status)?'Delete shipment and refund wallet':'Only booked or cancelled shipments can be deleted'} className="grid h-8 w-8 place-items-center rounded-lg text-red-500 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-25"><Trash2 size={15}/></button></td>}</tr>)}</tbody></table>{!shipments.length&&<div className="p-10 text-center text-sm text-slate-400">No shipments found.</div>}</div>
}

function Shipments({ shipments, user, onDelete }) { const [query,setQuery]=useState(''),[status,setStatus]=useState('All statuses');const filtered=shipments.filter(s=>(status==='All statuses'||s.status===status)&&Object.values(s).some(v=>String(v).toLowerCase().includes(query.toLowerCase())));const permissions=user.role==='agent'?['createBooking','deleteBooking']:(user.permissions||[]);return <div className="fade"><PageTitle eyebrow="Bookings" title="My shipments" text="Review every booking and its latest operational status." action={permissions.includes('createBooking')?<button onClick={()=>go('booking')} className="btn-primary"><Plus size={16}/> New booking</button>:null}/><div className="card overflow-hidden"><div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="relative w-full sm:max-w-xs"><Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input value={query} onChange={e=>setQuery(e.target.value)} className="field search-field py-2.5" placeholder="Search AWB, route or commodity"/></div><select value={status} onChange={e=>setStatus(e.target.value)} className="field w-full py-2.5 sm:w-40"><option>All statuses</option><option>Booked</option><option>In transit</option><option>Delivered</option><option>Cancelled</option></select></div><ShipmentTable shipments={filtered} onDelete={onDelete} canDelete={permissions.includes('deleteBooking')}/></div></div> }
...
function ApprovalRequired({user,onRefresh}){const [loading,setLoading]=useState(false);const refresh=async()=>{setLoading(true);await onRefresh(true);setLoading(false)};return <div className="fade"><PageTitle eyebrow="Account approval" title="Business verification pending" text="Your application is connected to Super Admin Agent Management and is waiting for review."/><section className="card overflow-hidden"><div className="bg-amber-50 p-6 sm:p-8"><span className="grid h-12 w-12 place-items-center rounded-full bg-white text-amber-700 shadow-sm"><Clock3 size={23}/></span><h2 className="font-display mt-5 text-xl font-semibold text-navy">Booking is locked until approval</h2><p className="mt-2 max-w-xl text-sm leading-6 text-slate-600">Application <strong>{user.id||user.email}</strong> has been submitted. A Super Admin can approve it from Agent Management; the updated status will sync back to this portal.</p></div><div className="grid gap-4 p-6 sm:grid-cols-3 sm:p-8">{[['1','Application saved','Your business details are stored securely.'],['2','Admin review','KYC and station access are reviewed.'],['3','Booking activated','AWB booking unlocks after approval.']].map(([number,title,text])=><div key={number} className="rounded-xl border border-slate-100 p-4"><span className="grid h-7 w-7 place-items-center rounded-full bg-sky text-[10px] font-bold text-brand">{number}</span><strong className="mt-3 block text-xs text-navy">{title}</strong><p className="mt-1 text-[10px] leading-5 text-slate-400">{text}</p></div>)}</div><div className="flex flex-col gap-3 border-t border-slate-100 p-6 sm:flex-row"><button onClick={refresh} disabled={loading} className="btn-primary">{loading?<LoaderCircle className="animate-spin" size={16}/>:<RefreshCcw size={16}/>} Check approval status</button><button onClick={()=>go('profile')} className="btn-secondary">View submitted profile</button></div></section></div>}

function Portal({ user, onLogout, toast, onRefreshUser }) {
  const employeeWallet={wallet:{email:user.ownerEmail||user.email,ownerName:user.name,businessName:user.business,balance:0,status:'Unavailable'},transactions:[]}
  const [route,setRoute]=useState(currentRoute()),[mobile,setMobile]=useState(false),[shipments,setShipments]=useState(()=>user.role==='employee'?[]:(JSON.parse(localStorage.getItem('aac_shipments')||'null')||defaultShipments)),[walletData,setWalletData]=useState(()=>user.role==='employee'?employeeWallet:(JSON.parse(localStorage.getItem('aac_wallet')||'null')||defaultWallet))
  useEffect(()=>{const change=()=>{const next=currentRoute();if(!routeMap[next]||!canAccessRoute(user,next))return go('dashboard');setRoute(next)};window.addEventListener('hashchange',change);change();return()=>window.removeEventListener('hashchange',change)},[user.role,user.permissions?.join(',')])
  useEffect(()=>{if(user.role==='employee'&&!(user.permissions||[]).includes('viewShipments'))return;api.get('/api/shipments').then(data=>{const rows=Array.isArray(data)?data:data?.shipments;if(Array.isArray(rows)){setShipments(rows);localStorage.setItem('aac_shipments',JSON.stringify(rows))}}).catch(()=>{})},[user.email])
  const refreshWallet=async(notify=false)=>{if(user.role==='employee'&&!(user.permissions||[]).includes('viewWallet'))return;try{const data=await api.get('/api/wallet');if(data?.wallet){setWalletData(data);localStorage.setItem('aac_wallet',JSON.stringify(data));if(notify)toast('Wallet balance refreshed.')}}catch(error){if(notify)toast(error.message||'Wallet could not be refreshed.')}}
  ...
  const deleteShipment=async shipment=>{if(!window.confirm(`Delete shipment ${shipment.awb}? Wallet-paid freight will be refunded.`))return;try{const result=await api.delete(`/api/bookings/${encodeURIComponent(shipment.awb)}`);setShipments(current=>{const next=current.filter(item=>item.awb!==shipment.awb);localStorage.setItem('aac_shipments',JSON.stringify(next));return next});if(result.wallet)onWalletChange(result.wallet,result.transaction);toast(`Shipment ${shipment.awb} deleted${result.wallet?' and wallet refunded':''}.`)}catch(error){toast(error.message||'Shipment could not be deleted.')}}
  const pendingApproval=/pending/i.test(String(user.status||''))
  let page=<Dashboard user={user} shipments={shipments} wallet={walletData.wallet}/>;if(route==='rates')page=<RateCalculator toast={toast}/>;if(route==='booking')page=pendingApproval?<ApprovalRequired user={user} onRefresh={onRefreshUser}/>:<Booking addShipment={addShipment} toast={toast} wallet={walletData.wallet} onWalletChange={onWalletChange}/>;if(route==='shipments')page=<Shipments shipments={shipments} user={user} onDelete={deleteShipment}/>;if(route==='track')page=<Track shipments={shipments}/>;if(route==='wallet'&&user.role==='agent')page=<WalletPage data={walletData} onWalletChange={onWalletChange} onRefresh={()=>refreshWallet(true)} toast={toast}/>;if(route==='documents')page=<Documents toast={toast}/>;if(route==='profile')page=<Profile user={user} toast={toast}/>
...
export default function App() {
  ...
  if(user)return <><Portal user={user} onLogout={logout} toast={setToast} onRefreshUser={refreshUser}/><Toast message={toast} onClose={()=>setToast('')}/></>
