...
import {
  ...
  Plus, Search, Send, Settings, ShieldCheck, Truck, Upload, User, UserPlus,
...
const calculateLocalFreight = (form,extraServices,controls=defaultFreightControls) => {
  ...
  return {weights:{totalActualWeight:Number(actual.toFixed(2)),totalVolumetricWeight:Number(volumetric.toFixed(2)),chargeableWeight:Number(chargeable.toFixed(2))},charges,total:Number(charges.reduce((sum,row)=>sum+row.amount,0).toFixed(2)),controls,flights:localFlightOptions(form.origin,form.destination,form.bookingDate)}
}
...
function Dashboard({ user, shipments }) {
  ...
  return <div className="fade">
    ...
    <section aria-label="Account metrics" className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
      {[[Box,'Active shipments',active,'2 require attention','bg-blue-50 text-brand'],[Plane,'In transit',inTransit,'Moving now','bg-cyan-50 text-cyan-600'],[PackageCheck,'Delivered',delivered,'This period','bg-green-50 text-green-600'],[Gauge,'Total cargo',`${cargoWeight.toLocaleString('en-IN')} kg`,'Booked weight','bg-purple-50 text-purple-600'],[CircleDollarSign,'Credit available','₹3.64 L','of ₹5.00 L limit','bg-amber-50 text-amber-600'],[Clock3,'On-time rate','98.4%','Last 30 days','bg-rose-50 text-rose-600']].map(([Icon,label,value,meta,color])=><article key={label} className="card p-4 sm:p-5"><div className="flex items-start justify-between"><span className={`grid h-10 w-10 place-items-center rounded-xl ${color}`}><Icon size={19}/></span><span className="text-[9px] font-bold text-green-600">LIVE</span></div><strong className="font-display mt-4 block text-xl text-navy">{value}</strong><span className="mt-1 block text-[11px] font-semibold text-slate-600">{label}</span><small className="mt-2 block text-[9px] text-slate-400">{meta}</small></article>)}
    </section>
    ...
    <div className="mt-6 grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
        <section className="card p-5 sm:p-6"><div className="flex items-start justify-between"><div><p className="text-[9px] font-bold uppercase tracking-[.15em] text-brand">Account</p><h2 className="font-display mt-1 text-base font-semibold text-navy">Credit & billing</h2></div><WalletCards size={20} className="text-brand"/></div><div className="mt-5 rounded-xl bg-slate-50 p-4"><div className="flex items-end justify-between"><div><span className="text-[9px] text-slate-400">Available credit</span><strong className="font-display mt-1 block text-xl text-navy">₹3.64 L</strong></div><span className="text-[10px] font-bold text-green-600">73% free</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full w-[27%] rounded-full bg-brand"/></div></div><dl className="mt-4 space-y-3 text-[10px]">{[['Credit limit','₹5.00 L'],['Outstanding','₹1.36 L'],['Next due date','29 Jul 2026']].map(([label,value])=><div key={label} className="flex justify-between"><dt className="text-slate-400">{label}</dt><dd className="font-bold text-navy">{value}</dd></div>)}</dl><button onClick={()=>go('profile')} className="mt-5 text-[10px] font-bold text-brand">View account details →</button></section>
...
function Booking({ addShipment, toast }) {
  ...
  const [form,setForm]=useState({origin:selected?.origin||'DEL',destination:selected?.destination||'DXB',bookingDate:today,commodity:'General cargo',service:selected?.carrier||'Alliance Priority',pieces:selected?.pieces||3,actualWeight:25,length:12,width:15,height:16,pickup:'',shipper:'',shipperAccount:'',consignee:'',consigneeAccount:'',instructions:'',carrierAgentName:'',agentCity:'',iataCode:'',carrierAccountNumber:''})
  const [extraServices,setExtraServices]=useState(['handleWithCare'])
  const [freight,setFreight]=useState(null),[calculating,setCalculating]=useState(false),[selectedFlight,setSelectedFlight]=useState(null)
  ...
  const update=e=>{
    ...
    setForm(current=>name==='consigneeAccount'?{...current,[name]:value,consignee:'',carrierAgentName:'',agentCity:'',iataCode:'',carrierAccountNumber:''}:{...current,[name]:value})
  ...
  const submit=e=>{
    ...
    addShipment(booking);api.post('/api/bookings',{...form,...booking,extraServices,freightSummary:freight,selectedFlight}).catch(()=>{});localStorage.removeItem('aac_selected_rate');toast(`Booking confirmed on ${selectedFlight.flightNumber}. AWB ${awb}`);go('shipments')
  }
  ...
  const bookingReady=Boolean(freight&&selectedFlight&&lookup.status==='found')
  ...
  return <div className="fade"><PageTitle eyebrow="New shipment" title="Plan and book air cargo" text="Enter cargo details, calculate the complete freight, choose an available flight and confirm your booking."/>
    <div className="mb-6 grid grid-cols-2 gap-2 sm:grid-cols-4">{[['1','Cargo details',true],['2','Freight summary',Boolean(freight)],['3','Select flight',Boolean(selectedFlight)],['4','Confirm',bookingReady]].map(([step,label,active])=><div key={step} className={`flex items-center gap-3 rounded-xl border p-3 ${active?'border-blue-200 bg-sky':'border-slate-200 bg-white'}`}><span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-[10px] font-bold ${active?'bg-brand text-white':'bg-slate-100 text-slate-400'}`}>{active?<Check size={13}/>:step}</span><span className={`text-[10px] font-bold ${active?'text-navy':'text-slate-400'}`}>{label}</span></div>)}</div>
...
function Portal({ user, onLogout, toast }) {
  const [route,setRoute]=useState(currentRoute()),[mobile,setMobile]=useState(false),[shipments,setShipments]=useState(()=>JSON.parse(localStorage.getItem('aac_shipments')||'null')||defaultShipments)
  ...
  const addShipment=s=>{const next=[s,...shipments];setShipments(next);localStorage.setItem('aac_shipments',JSON.stringify(next))}
  let page=<Dashboard user={user} shipments={shipments}/>;if(route==='rates')page=<RateCalculator toast={toast}/>;if(route==='booking')page=<Booking addShipment={addShipment} toast={toast}/>;if(route==='shipments')page=<Shipments shipments={shipments}/>;if(route==='track')page=<Track shipments={shipments}/>;if(route==='documents')page=<Documents toast={toast}/>;if(route==='profile')page=<Profile user={user} toast={toast}/>
  return <div className="min-h-screen bg-mist"><Sidebar route={route} mobile={mobile} setMobile={setMobile} onLogout={onLogout} user={user}/><PortalHeader route={route} setMobile={setMobile} user={user}/><main className="p-4 sm:p-7 lg:ml-[270px] lg:p-8 xl:p-10">{page}</main></div>
}
...
export default function App() {
  ...
  const logout=()=>{api.post('/api/auth/logout',{}).catch(()=>{});localStorage.removeItem('aac_session');setUser(null);go('login')}
